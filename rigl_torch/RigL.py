    ...
    def __init__(self, model, optimizer, dense_allocation=1, T_end=None, sparsity_distribution='uniform', ignore_linear_layers=True, delta=100, alpha=0.3, static_topo=False, grad_accumulation_n=1, state_dict=None):
        ...
        if state_dict is not None:
            ...
            if sparsity_distribution == 'uniform':
                ...
                epsilon = dense_allocation * total / denom
                self.S = [1 - epsilon*(param_dim_sums[i])/(full_params[i]) for i in range(len(full_params))]
                print("dense_allocation:", dense_allocation)
                ...
                for layer in model.modules():
                    if type(layer)==(nn.Conv2d) or type(layer)==(nn.Linear):
                        full_params.append(np.product(layer.weight.shape))
                target_non_zeros = dense_allocation*sum(full_params)
                ...
                self.S = [1-i for i in np.minimum(np.divide(non_zeros_per_layer, full_params), 1)]
                print("self.S:", self.S)
